...
CREATE VIEW VPendencia_Cliente AS SELECT p.idPendencia, c.nome, c.idCliente AS id, p.motivo, p.aberto FROM  pendencia_cliente AS p JOIN cliente AS c ON pc.idCliente = c.idCliente;
CREATE VIEW VPendencia_Veiculo AS SELECT pv.idPendencia_Veiculo, CONCAT(mar.nomeMarca, " ", m.nomeModelo) AS nome, v.idVeiculo AS id, pv.motivo, pv.aberto FROM pendencia_veiculo AS pv JOIN veiculo AS v ON pv.idVeiculo = v.idVeiculo join modelo AS m ON m.idModelo = v.idModelo JOIN marca AS mar ON m.idMarca = mar.idMarca;
